...
open class FirExpressionsResolveTransformer(transformer: FirBodyResolveTransformer) : FirPartialBodyResolveTransformer(transformer) {
    ...
    override fun transformTypeOperatorCall(
        ...
        transformTypeOperatorCallChildren(resolved, data)
    ...
    private fun transformTypeOperatorCallChildren(
        resolved: FirTypeOperatorCall,
        data: ResolutionMode
    ) {
        if (resolved.operation == FirOperation.AS || resolved.operation == FirOperation.SAFE_AS) {
            val argument = resolved.argumentList.arguments.singleOrNull() ?: error("Not a single argument: ${resolved.render()}")

            if (argument is FirFunctionCall) {
                val expectedType = resolved.conversionTypeRef.coneTypeSafe<ConeKotlinType>()?.takeIf {
                    // is not bare type
                    ...
                    if (resolved.operation == FirOperation.SAFE_AS)
                ...
                if (expectedType != null) {
                    val newMode = ResolutionMode.WithExpectedTypeFromCast(resolved.conversionTypeRef.withReplacedConeType(expectedType))
                    argument.transform<FirElement, ResolutionMode>(transformer, newMode)
                    resolved.transformAnnotations(transformer, data)
                    return
                }
            }
        }

        resolved.transformOtherChildren(transformer, ResolutionMode.ContextIndependent)
    }
